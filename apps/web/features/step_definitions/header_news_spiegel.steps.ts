...
Given(
  'die API liefert mindestens {int} Einträge',
...
Then(
  'jede Headline zeigt den Titel und das Veröffentlichungsdatum',
  async function () {
...
Then(
  'ein Klick auf eine Headline öffnet den Artikel in einem neuen Tab',
  async function () {
...
Then(
  'werden genau {int} Headlines angezeigt \\(keine Platzhalter für fehlende)',
  async function (count: number) {
    // const headlines = await page.locator('[data-testid="news-headline-item"]');
    // await expect(headlines).toHaveCount(count);
...
Given(
  'ein Eintrag hat das UTC-Datum {string}',
  async function (utcDate: string) {
    apiResponse = {
      headlines: [
        {
...
Then(
  'wird das Datum in Europe\\/Berlin als {string} angezeigt',
