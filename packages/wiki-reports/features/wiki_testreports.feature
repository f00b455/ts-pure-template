Feature: Testberichte in GitHub Wiki veröffentlichen
  ...
  Scenario: Erfolgreiche Veröffentlichung eines Laufes
    ...
    Then werden HTML-Reports unter "wiki/reports/main/123/" abgelegt
    And die Wiki-Startseite verlinkt "Letzter Lauf: 123 (abc123)"
  ...
  Scenario: Getrennte Verzeichnisse pro Branch
    ...
    When die Publish-Stage ausgeführt wird
    Then werden Reports unter "wiki/reports/feature-x/456/" gespeichert
  ...
  Scenario: Kein Report vorhanden
    ...
    And die Pipeline schlägt nicht wegen des Wiki-Schritts fehl

  @retention
  Scenario: Aufräumen alter Reports
    Given mehr als 20 Läufe existieren für einen Branch
    When die Publish-Stage läuft
    Then werden ältere Verzeichnisse entfernt
    And die Wiki-Größe bleibt kontrolliert